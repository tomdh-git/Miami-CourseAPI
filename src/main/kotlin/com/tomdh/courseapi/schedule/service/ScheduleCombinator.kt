...
class ScheduleCombinator(private val cache: FillerAttributeCache) {
    ...
    suspend fun getScheduleByCourses(input: ScheduleByCourseInput, connector: SchoolConnector): List<Schedule> {
        ...
        val combinatorGroups = valid.values.map { CourseCombinatorMapper.mapToCombinatorGroup(it) }

        val combinatorResults = IntervalCombinator.generate<Course> {
            groups = combinatorGroups
            constraints {
        ...
        return combinatorResults.map { result ->
            Schedule(courses = result.items, freeTime = result.freeTimeMinutes)
        }
    }
    ...
    suspend fun getFillerByAttributes(input: FillerByAttributesInput, connector: SchoolConnector): List<Schedule> = coroutineScope {
        ...
        schedules.map { schedule ->
            val existingCombinatorItems = CourseCombinatorMapper.mapToCombinatorGroup(schedule.courses)

            val compatibleFillers = IntervalCombinator.findFillers<Course> {
                existing = existingCombinatorItems
            ...
            if (compatibleFillers.isNotEmpty()) {
                val bestFiller = compatibleFillers.first().payload
                Schedule(courses = schedule.courses + bestFiller, freeTime = schedule.freeTime)
            } else {
                Schedule(courses = schedule.courses, freeTime = schedule.freeTime)
            }
        }
    }
    ...
    private suspend fun fetchCourses(parsed: List<Pair<String, String>>, input: ScheduleByCourseInput, connector: SchoolConnector): Map<Pair<String, String>, List<Course>> = coroutineScope {
        return@coroutineScope parsed.map { (subject, num) ->
            async {
                val inputQuery = CourseByInfoInput(
                    school = input.school,
                ...
                val sections = connector.getCourseByInfo(inputQuery)
                Pair(Pair(subject, num), sections)
            }
        }.awaitAll().groupBy(
            { it.first },
            { it.second }
